...
KEYCLOAK_CLIENT_SECRET = os.getenv("KEYCLOAK_CLIENT_SECRET", "sharednotes-api-secret-key-2024")
...
async def verify_token(credentials: HTTPAuthorizationCredentials = Depends(security)) -> dict:
    ...
    try:
        ...
        payload = jwt.decode(
            ...
            signing_key.key,
            algorithms=["RS256"],
            options={"verify_aud": False, "verify_iss": False}  # Skip audience and issuer validation for now
        )
...
async def proxy_request(request: Request, target_url: str, backend_path: str, user: dict = Depends(get_current_user)):
...
# Notes service routes - handle both with and without trailing slash
...
async def notes_proxy(request: Request, path: str, user: dict = Depends(get_current_user)):
    return await proxy_request(request, NOTES_SERVICE_URL, f"/notes/{path}", user)

# Search routes (now handled by notes-service)
...
async def search_proxy(request: Request, path: str, user: dict = Depends(get_current_user)):
...
async def tags_proxy(request: Request, path: str, user: dict = Depends(get_current_user)):
...
async def share_proxy(request: Request, path: str, user: dict = Depends(get_current_user)):
    return await proxy_request(request, SHARE_SERVICE_URL, f"/share/{path}", user)

if __name__ == "__main__":
    import uvicorn
    
    # HTTP internal server - accessed through frontend nginx proxy
    logger.info("Starting API Gateway on HTTP port 8000 (internal)")
